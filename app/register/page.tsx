...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    // 2. Salvar na tabela clients
    if (authData.user) {
      const { error: clientError } = await supabase.from('clients').insert([{
        id: authData.user.id, // ligar o auth id com a tabela (opcional se quiser usar uuid direto)
        email: email,
      ...
      if (clientError) {
         console.error("Erro ao salvar perfil do cliente", clientError);
      }
    }
  ...
  return (
    <div className="min-h-screen bg-[#191A21] flex flex-col items-center justify-center p-6 text-white font-['Helvetica',_sans-serif]">
      <div className="w-full max-w-md bg-[#242628] border border-zinc-800 p-10 shadow-2xl relative mt-10">
        ...
        <form onSubmit={handleRegister} className="space-y-6">
          ...
          <div>
            <label className="block text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-2">WhatsApp (Opcional)</label>
            <input 
              type="tel"
              placeholder="+55 11 90000-0000"
              className="w-full bg-transparent border-b border-zinc-700 py-2 text-sm text-white outline-none focus:border-[#A9AFDE] transition-colors placeholder:text-zinc-700"
              value={whatsapp}
          ...
          <div>
            <label className="block text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-2">Criar Senha</label>
            <input 
              required
              type="password"
              className="w-full bg-transparent border-b border-zinc-700 py-2 text-sm text-white outline-none focus:border-[#A9AFDE] transition-colors"
        ...
        <div className="mt-8 text-center text-[10px] tracking-widest uppercase text-zinc-500">
          <p>Já possui uma credencial?</p>
          <Link href="/login" className="text-[#A9AFDE] hover:text-white mt-1 inline-block transition-colors font-bold border-b border-[#A9AFDE]/30 pb-0.5">
            Acessar Acervo
          </Link>
        </div>
      </div>
